...
import math, torch, torchaudio, os
try:
    import torch_musa
...
class ECAPA_TDNN(nn.Module):
    ...
    def forward(self, x, aug):
        ...
        x_tmp = torch.var(x.to('cpu'),dim=2,keepdim=True).to(DEVICE)
        global_x = torch.cat((x,torch.mean(x,dim=2,keepdim=True).repeat(1,1,t), torch.sqrt(x_tmp.clamp(min=1e-4)).repeat(1,1,t)), dim=1)
